...
@Component({
  selector: 'app-judicial-acts',
  templateUrl: './judicial-acts.component.html',
  styleUrls: ['./judicial-acts.component.css']
})
export class JudicialActsComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.JA, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.JA).subscribe(response => {
