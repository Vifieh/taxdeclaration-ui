...
@Component({
  selector: 'app-purchase-of-stamps',
  templateUrl: './purchase-of-stamps.component.html',
  styleUrls: ['./purchase-of-stamps.component.css']
})
export class PurchaseOfStampsComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.POS, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.POS).subscribe(response => {
