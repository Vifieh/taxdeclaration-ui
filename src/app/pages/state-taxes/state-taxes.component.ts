...
@Component({
  selector: 'app-state-taxes',
  templateUrl: './state-taxes.component.html',
  styleUrls: ['./state-taxes.component.css']
})
export class StateTaxesComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.ST, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.ST).subscribe(response => {
