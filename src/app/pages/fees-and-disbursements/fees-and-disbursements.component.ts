...
@Component({
  selector: 'app-fees-and-disbursements',
  templateUrl: './fees-and-disbursements.component.html',
  styleUrls: ['./fees-and-disbursements.component.css']
})
export class FeesAndDisbursementsComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.FAD, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.FAD).subscribe(response => {
