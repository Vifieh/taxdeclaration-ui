...
@Component({
  selector: 'app-real-estate-changes',
  templateUrl: './real-estate-changes.component.html',
  styleUrls: ['./real-estate-changes.component.css']
})
export class RealEstateChangesComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.REC, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.REC).subscribe(response => {
