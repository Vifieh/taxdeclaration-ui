...
@Component({
  selector: 'app-land-royalty',
  templateUrl: './land-royalty.component.html',
  styleUrls: ['./land-royalty.component.css']
})
export class LandRoyaltyComponent implements OnInit, OnDestroy {
  ...
  declareTaxForm = this.formBuilder.group({
      ...
      typeOfTax: [TaxType.LR, [Validators.required]]
    }
  );
  ...
  getTaxesDeclaredByUser() {
    const declaredTaxListSub = this.declareTaxService.getTaxesDeclaredByUser(TaxType.LR).subscribe(response => {
